      ...
      function init() {
        ...
        .then(data => {
          ...
          calendarEl.forEach(calendarElement => {
                          ...
                          <div class="elCalendar__body">
                            ${month.firstDates.map(el => `
                              <div
                                ...
                                ${checkHoliday(month.monthName, el.dayNum, nbOfLeaves) ? ' bg-accent-1' : ''}""
                              >
                                <span class="js-date">
                                  ${el.dayNum}
                                </span>
            ...
            function checkHoliday(monthName, dayNum, leaveDays) {
              // Create a Date object for the given monthName and dayNum
              const startDate = new Date(`${monthName} ${dayNum}, ${currentYear}`);
              let isPHoliday = false;
            
              // Add leaveDays to the start date
              const targetDate = new Date(startDate);
              targetDate.setDate(targetDate.getDate() + leaveDays);
            
              // Convert targetDate back to monthName and dayNum
              const targetMonthName = targetDate.toLocaleString('default', { month: 'long' });
              const targetDayNum = targetDate.getDate();
            
              // Check if the target date is a public holiday
              isPHoliday = isPublicHoliday(targetMonthName, targetDayNum);
            
              if (leaveDays == 2) {
                const targetClosestDate = new Date(startDate);
                targetClosestDate.setDate(targetClosestDate.getDate() + 1);
                const targetClosestMonthName = targetClosestDate.toLocaleString('default', { month: 'long' });
                const targetClosestDayNum = targetClosestDate.getDate();
            
                if (isPublicHoliday(targetClosestMonthName, targetClosestDayNum) && !isPHoliday) {
                  return true;
                } 
            
                if (isPHoliday && isWeekend(new Date(targetDate.setDate(targetDate.getDate() + 1)))) {
                  // set the next monday as true
                  const nextMonday = new Date(targetDate);
                  nextMonday.setDate(nextMonday.getDate() + (7 - nextMonday.getDay() + 1));
                  isPHoliday = isPublicHoliday(nextMonday.toLocaleString('default', { month: 'long' }), nextMonday.getDate());
                  return isPHoliday;
                }
              }
            
              // Check if the target date is a weekend
